    ...
    const std::type_info *const *types, size_t &type_index, size_t &arg_index)
{
	...
	// `is_return_value.top()` is true if we are currently inside the return
	// type of the signature. Using `@^`/`@$` we can force types to be
	// arg/return types while `@!` pops back to the previous state.
	...
	for (const auto *pc = type_caster_name_field; *pc != '\0'; ++pc)
	{
		...
		if (c == '{')
		{
			...
			if (arg_index < func_rec->args.size() &&
			    func_rec->args[arg_index].name)
			{
			...
			else
			{
				signature +=
				    "arg" +
				    std::to_string(arg_index - (func_rec->is_method ? 1 : 0));
			}
			signature += ": ";
		}
		else if (c == '}')
		...
		else if (c == '%')
		{
			...
			else if (func_rec->is_new_style_constructor && arg_index == 0)
			{
				...
				signature +=
				    func_rec->scope.attr("__module__").cast<std::string>() +
				    "." +
		...
		{
			// `@^ ... @!` and `@$ ... @!` are used to force arg/return value
			// type (see
			...
			// in an argument or a return value position (see io_name<text1,
			// text2>). For named arguments (py::arg()) with noconvert set,
			// return value type is used.
			++pc;
...
class cpp_function : public function
{
  public:
	...
	{
		initialize([f](Class *c, Arg... args) -> Return
	...
	{
		initialize([f](const Class *c, Arg... args) -> Return
		           { return (c->*f)(std::forward<Arg>(args)...); },
  ...
  protected:
	...
	void initialize(Func &&f, Return (*)(Args...), const Extra &...extra)
	{
		...
		static_assert(expected_num_args<Extra...>(sizeof...(Args),
		                                          cast_in::args_pos >= 0,
		                                          cast_in::has_kwargs),
		...
		rec->nargs_pos = cast_in::args_pos >= 0
		                     ...
		                     : sizeof...(Args) -
		                           cast_in::has_kwargs; // Will get reduced more
...

		{
			constexpr bool has_kw_only_args =
			               ...
			               has_arg_annotations =
			                   any_of<is_keyword<Extra>...>::value;
			constexpr bool has_is_method =
			    any_of<std::is_same<is_method, Extra>...>::value;
			// The implicit `self` argument is not present and not counted in
			// method definitions.
			constexpr bool has_args = cast_in::args_pos >= 0;
			constexpr bool is_method_with_self_arg_only =
			    has_is_method && !has_args;
			static_assert(
			    has_arg_annotations || !has_kw_only_args,
			    "py::kw_only requires the use of argument annotations");
			static_assert(((/* Need `py::arg("arg_name")` annotation in
			                   function/method. */
			                has_arg_annotations) ||
			               (/* Allow methods with no arguments `def method(self,
			                 * /): ...`. A method has at least one argument
			                 * `self`. There can be no `py::arg` annotation.
			                 * E.g. `class.def("method", py::pos_only())`.
			                 */
			                is_method_with_self_arg_only)) ||
			                  !has_pos_only_args,
			              "py::pos_only requires the use of argument "
			              "annotations (for docstrings "
			...
			constexpr auto kw_only_pos =
			    constexpr_first<is_kw_only, Extra...>();
			constexpr auto pos_only_pos =
			    constexpr_first<is_pos_only, Extra...>();
    ...
    !defined(PYBIND11_DISABLE_NEW_STYLE_INIT_WARNING)
		if (rec->is_constructor && !rec->is_new_style_constructor)
		{
			...
			PyErr_WarnEx(PyExc_FutureWarning,
			             ("pybind11-bound class '" + class_name +
			              ...
			              "the upgrade guide in pybind11's docs. This message "
			              "is only visible "
		...
		if (rec->sibling)
		{
			if (PyCFunction_Check(rec->sibling.ptr()))
			{
				...
				{
					/* Never append a method to an overload chain of a parent
					   class; instead, hide the parent's overloads in this case
					 */
					chain = nullptr;
				}
			}
		...
		if (!chain)
		{
			...
			detail::
			    function_record_PyTypeObject_PyType_Ready(); // Call-once
			                                                 // initialization.
			object py_func_rec = detail::function_record_PyObject_New();
			((detail::function_record_PyObject *)py_func_rec.ptr())
			    ->cpp_func_rec = unique_rec.release();
			guarded_strdup.release();

			object scope_module = detail::get_scope_module(rec->scope);
			m_ptr = PyCFunction_NewEx(rec->def, py_func_rec.ptr(),
			                          scope_module.ptr());
			if (!m_ptr)
			{
		...
		else
		{
			...
			if (chain->is_method != rec->is_method)
			{
				pybind11_fail("overloading a method with both static and "
...
#if !defined(PYBIND11_DETAILED_ERROR_MESSAGES)
				              "#define PYBIND11_DETAILED_ERROR_MESSAGES or "
				              "compile in debug mode for more "
				              "details"
#else
				              ...
				              " method " +
				              std::string(
				                  pybind11::str(rec->scope.attr("__name__"))) +
			...
			if (rec->prepend)
			{
				...
				rec->next = chain;
				auto *py_func_rec =
				    (detail::function_record_PyObject *)PyCFunction_GET_SELF(
				        m_ptr);
	...
	friend void
	detail::function_record_PyTypeObject_methods::tp_dealloc_impl(PyObject *);
...
// See https://github.com/python/cpython/pull/22670
	                            ...
	                            PyObject *kwargs_in)
	{
		...
		const function_record *overloads =
		    function_record_ptr_from_PyObject(self);
		...
		if (overloads->is_constructor)
		{
			                        ...
			                        (PyTypeObject *)overloads->scope.ptr()))
			{
				set_error(PyExc_TypeError,
				          "__init__(self, ...) called with invalid or missing "
				          "`self` argument");
				return nullptr;
			}
			     ...
			     current_overload = current_overload->next)
			{
				...
				if (args_copied < num_args)
				{
					...
					for (; args_copied < num_args; ++args_copied)
					{
						...
						{
							value =
							    dict_getitemstring(kwargs.ptr(), arg_rec.name);
						}
						...
						if (value)
						{
							...
							if (PyDict_DelItemString(kwargs.ptr(),
							                         arg_rec.name) == -1)
							{
...
class module_ : public object
{
  public:
	...
	static module_ create_extension_module(
	    const char *name, const char *doc, module_def *def,
	...
	/** \rst
	    ...
	    ``def`` should point to a statically allocated module_def.
	    ``slots`` must already contain a Py_mod_exec or Py_mod_create slot and
	will be filled with additional slots from the supplied options (and the
	empty sentinel slot). \endrst */
	template <typename... Options>
	static object
	initialize_multiphase_module_def(const char *name, const char *doc,
	                                 module_def *def, slots_array &slots,
	                                 Options &&...options)
	{
		size_t next_slot = 0;
		...
		{
			pybind11_fail(
			    "initialize_multiphase_module_def: not enough space in slots");
		}
		slots[next_slot++] = {Py_mod_multiple_interpreters,
		                      detail::multi_interp_slot(options...)};
#endif
		...
		if (detail::gil_not_used_option(options...))
		{
			...
			if (next_slot >= term_slot)
			{
				pybind11_fail("initialize_multiphase_module_def: not enough "
				              "space in slots");
			}
		...
		if (m == nullptr)
		{
			...
			pybind11_fail("Internal error in "
			              "module_::initialize_multiphase_module_def()");
		}
...
class generic_type : public object
{
  ...
  protected:
	                              ...
	                              detail::function_record *rec_func)
	{
		...
		const auto has_doc = (rec_func != nullptr) &&
		                     (rec_func->doc != nullptr) &&
...
template <typename Derived, typename Return, typename Class, typename... Args>
auto method_adaptor(Return (Class::*pmf)(Args...)
                        const) -> Return (Derived::*)(Args...) const
{
	static_assert(detail::is_accessible_base_of<Class, Derived>::value,
	              "Cannot bind an inaccessible base class method; use a lambda "
	              "definition instead");
	...
	static cpp_function readonly(PM pm, const handle &hdl)
	{
		...
		if (tinfo->holder_enum_v == holder_enum_t::smart_holder)
		{
			return cpp_function(
			    ...
			    {
				    std::shared_ptr<T> c_sp = type_caster<std::shared_ptr<T>>::
				        shared_ptr_with_responsible_parent(c_hdl);
				    D ptr = (*c_sp).*pm;
...
struct property_cpp_function_sh_unique_ptr_member
{
	...
	{
		static_assert(
		    !is_instantiation<std::unique_ptr, D>::value,
...
class class_ : public detail::generic_type
{
  ...
  public:
	...
	static_assert(!has_alias || std::is_polymorphic<type>::value,
	              "Cannot use an alias class (aka trampoline) with a "
	...
	static_assert(
	    ...
	    " pybind11::trampoline_self_life_support only if used in combination "
	    "with"
	    " pybind11::smart_holder");
		...
		install_buffer_funcs(
		    [](PyObject *obj, void *ptr) -> buffer_info *
		    {
			    ...
			    return new buffer_info(
			        ((capture *)ptr)->func(std::move(caster)));
		    },
		    ptr);
		weakref(m_ptr, cpp_function(
		                   [ptr](handle wr)
		                   {
			                   delete ptr;
			                   wr.dec_ref();
	                     ...
	                     const Extra &...extra)
	{
		...
		def_property_readonly(
		    name, property_cpp_function<type, D>::readonly(pm, *this),
		    return_value_policy::reference_internal, extra...);
		return *this;
	}
	...
	{
		return def_property(
		    name, fget, cpp_function(method_adaptor<type>(fset), is_setter()),
		    extra...);
	}
	template <typename Getter, typename... Extra>
	class_ &def_property(const char *name, const Getter &fget,
  ...
  private:
	                          ...
	                          const void *holder_const_void_ptr)
	{
		             ...
		             uninitialized_location, value_ptr_w_t, value_ptr_w_t))
		{
			...
			{
				new (uninitialized_location) holder_type(
				    holder_type::from_raw_ptr_unowned(value_ptr_w_t));
			}
		}
		v_h.set_holder_constructed();
	}
	...
	// operator delete. NOTE: The Python error indicator needs to cleared BEFORE
	// this function is called. This is because we could be deallocating while
	// cleaning up after a Python exception. If the error indicator is not
	// cleared but the C++ destructor code makes Python C API calls, those calls
	// are likely to generate a new exception, and pybind11 will then throw
	// `error_already_set` from the C++ destructor. This is forbidden and will
	...
	static void
	dealloc_release_gil_before_calling_cpp_dtor(detail::value_and_holder &v_h)
	{
		error_scope scope;
...
struct enum_base
{
	...
	PYBIND11_NOINLINE void init(bool is_arithmetic, bool is_convertible)
	{
		...
		m_base.attr("name") = property(cpp_function(
		    &enum_name, name("name"), is_method(m_base), pos_only()));
		...
		if (is_convertible)
		{
			...
			if (is_arithmetic)
			{
				...
				m_base.attr("__invert__") = cpp_function(
				    [](const object &arg) { return ~(int_(arg)); },
...
template <typename Type> class enum_ : public class_<Type>
{
  public:
	...
	{
		{
			...
			{
				pybind11_fail(
				    "pybind11::enum_ \"" + std::string(name) +
	...
	{
		m_base.value(name, pybind11::cast(value, return_value_policy::copy),
		             doc);
		return *this;
	}
                                  ...
                                  const type_info *this_type, const char *name)
{
	...
	bool not_overridden = with_internals(
	    [&key](internals &internals)
	    {
		    auto &cache = internals.inactive_override_cache;
...
template <class T> function get_override(const T *this_ptr, const char *name)
{
	...
	return tinfo ? detail::get_type_override(this_ptr, tinfo, name)
	             : function();
}
