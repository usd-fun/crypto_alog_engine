...
// fashion. See PR #4762.
inline void
all_type_info_add_base_most_derived_first(std::vector<type_info *> &bases,
                                          type_info *addl_base)
{
		    ...
		    : inst{inst}, types{tinfo}
		{
			if (inst != nullptr)
			{
				...
				curr = value_and_holder(inst /* instance */,
				                        (*types)[0] /* type info */,
				                        0, /* vpos: (non-simple types only): the
				                              first vptr comes first */
				                        0 /* index */);
			}
		}
...
struct value_and_holder_helper
{
	...
	{
		auto *vptr_gd_ptr =
		    std::get_deleter<memory::guarded_delete>(holder().vptr);
...
handle smart_holder_from_unique_ptr(
    std::unique_ptr<T, D> &&src, return_value_policy policy, handle parent,
    const std::pair<const void *, const type_info *> &st)
{
	if (policy == return_value_policy::copy)
	{
		throw cast_error(
		    "return_value_policy::copy is invalid for unique_ptr.");
	}
	if (!src)
	{
		return none().release();
	        ...
	        find_registered_python_instance(src_raw_void_ptr, tinfo))
	{
		...
		if (self_life_support != nullptr)
		{
			...
			if (v_h.inst != nullptr && v_h.vh != nullptr)
			{
				...
				// Critical transfer-of-ownership section. This must stay
				// together.
	...
	{
		// This is a multiple-inheritance situation that is incompatible with
		// the current shared_from_this handling (see PR #3023). Is there a
		// better solution?
		src_raw_void_ptr = nullptr;
	}
	auto smhldr =
	    smart_holder::from_unique_ptr(std::move(src), src_raw_void_ptr);
...
template <typename T, typename D>
handle smart_holder_from_unique_ptr(
    std::unique_ptr<T const, D> &&src, return_value_policy policy,
...
template <typename T>
handle smart_holder_from_shared_ptr(
    const std::shared_ptr<T> &src, return_value_policy policy, handle parent,
    const std::pair<const void *, const type_info *> &st)
{
	switch (policy)
	{
...
template <typename T>
handle smart_holder_from_shared_ptr(
    const std::shared_ptr<T const> &src, return_value_policy policy,
    handle parent, const std::pair<const void *, const type_info *> &st)
{
                                                 ...
                                                 std::unique_ptr<D> &&deleter)
{
	if (deleter == nullptr)
	{
		pybind11_fail("smart_holder_type_casters: deleter is not default "
		              "constructible and no"
...
template <typename T> struct load_helper : value_and_holder_helper
{
	...
	make_shared_ptr_with_responsible_parent(T *raw_ptr, handle parent)
	{
	...
	std::shared_ptr<T>
	load_as_shared_ptr(void *void_raw_ptr, handle responsible_parent = nullptr,
	                   ...
	                   bool force_potentially_slicing_shared_ptr = false) const
	{
		...
		{
			auto *vptr_gd_ptr =
			    std::get_deleter<memory::guarded_delete>(hld.vptr);
			if (vptr_gd_ptr != nullptr)
			{
				std::shared_ptr<void> released_ptr =
				    vptr_gd_ptr->released_ptr.lock();
				if (released_ptr)
				{
			...
			auto *sptsls_ptr =
			    std::get_deleter<shared_ptr_trampoline_self_life_support>(
			        hld.vptr);
			if (sptsls_ptr != nullptr)
			{
				// This code is reachable only if there are multiple
				// registered_instances for the same pointee.
				if (reinterpret_cast<PyObject *>(loaded_v_h.inst) ==
				    sptsls_ptr->self)
				{
					pybind11_fail("smart_holder_type_caster_support "
					              "load_as_shared_ptr failure: "
			...
			if (hld.vptr_is_external_shared_ptr)
			{
				pybind11_fail(
				    ...
				    "implemented: trampoline-self-life-support for external "
				    "shared_ptr "
				    "to type inheriting from std::enable_shared_from_this.");
			}
			pybind11_fail("smart_holder_type_casters: load_as_shared_ptr "
			              "failure: internal inconsistency.");
		}
	...
	template <typename D>
	std::unique_ptr<T, D>
	load_as_unique_ptr(void *raw_void_ptr,
	                   const char *context = "load_as_unique_ptr")
	{
		...
		std::unique_ptr<D> extracted_deleter =
		    holder().template extract_deleter<T, D>(context);
		...
		auto result = unique_with_deleter<T, D>(raw_type_ptr,
		                                        std::move(extracted_deleter));
		if (self_life_support != nullptr)
		{
		...
		else
		{
			...
			deregister_instance(loaded_v_h.inst, value_void_ptr,
			                    loaded_v_h.type);
		}
		// Critical section end.
	...
	// This assumes load_as_shared_ptr succeeded(), and the returned shared_ptr
	// is still alive. The returned unique_ptr is meant to never expire (the
	// behavior is undefined otherwise).
	template <typename D>
	std::unique_ptr<T, D>
	load_as_const_unique_ptr(T *raw_type_ptr,
...
public:
	     ...
	     const void *existing_holder = nullptr)
	{
		...
		if (handle registered_inst =
		        find_registered_python_instance(src, tinfo))
		{
		...
		case return_value_policy::copy:
			...
			else
			{
				...
				throw cast_error("return_value_policy = copy, but type is "
				                 "non-copyable! (#define "
				                 "PYBIND11_DETAILED_ERROR_MESSAGES or "
	...
	void load_value(value_and_holder &&v_h)
	{
		...
		{
			smart_holder_type_caster_support::value_and_holder_helper
			    v_h_helper;
			v_h_helper.loaded_v_h = v_h;
			if (v_h_helper.have_holder())
			{
				v_h_helper.throw_if_uninitialized_or_disowned_holder(
				    cpptype->name());
	...
	PYBIND11_NOINLINE bool try_load_foreign_module_local(handle src)
	{
		...
		if (auto *result = foreign_typeinfo->module_local_load(
		        src.ptr(), foreign_typeinfo))
		{
			value = result;
			return true;
		}
...
public:
	...
	src_and_type(const itype *src)
	{
		...
		const void *vsrc =
		    polymorphic_type_hook<itype>::get(src, instance_type);
...
protected:
	...
	template <typename T,
	          typename = enable_if_t<is_move_constructible<T>::value>>
