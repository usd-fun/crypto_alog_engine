...
static PyTypeObject function_record_PyTypeObject = {
    ...
    /* const char *tp_name */ function_record_PyTypeObject_methods::
        tp_name_impl,
...
{
	auto *py_func_rec =
	    PyObject_New(function_record_PyObject, &function_record_PyTypeObject);
...
{
	pybind11_fail("UNEXPECTED CALL OF "
	              "function_record_PyTypeObject_methods::tp_alloc_impl");
	// return nullptr; // Unreachable.
}
...
{
	// Deliberately ignoring the arguments for simplicity (expected is
	// `protocol: int`).
	const function_record *rec = function_record_ptr_from_PyObject(self);
	if (rec == nullptr)
	{
		pybind11_fail("FATAL: function_record_PyTypeObject reduce_ex_impl(): "
		              "cannot obtain cpp_func_rec.");
	}
		...
		if (scope_module)
		{
			...
			auto reconstruct_args =
			    make_tuple(str("__import__('importlib').import_module('") +
			               scope_module + str("')"));
			return make_tuple(std::move(builtins_eval),
			                  std::move(reconstruct_args))
			    .release()
			    .ptr();
		}
	}
