                                             ...
                                             PyObject *value)
{
	...
	const auto call_descr_set =
	    (descr != nullptr) && (value != nullptr) &&
	...
	with_internals(
	    [obj](internals &internals)
	    {
		        ...
		        found_type->second[0]->type == type)
		    {
			    ...
			    // Actually just `std::erase_if`, but that's only available in
			    // C++20
...
inline void clear_patients(PyObject *self)
{
	...
	with_internals(
	    [&](internals &internals)
	    {
		    ...
		    {
			    pybind11_fail("FATAL: Internal consistency check failed: "
			                  "Invalid clear_patients() call.");
		    }
                                         ...
                                         int flags)
{
	...
	if (info == nullptr)
	{
		pybind11_fail("FATAL UNEXPECTED SITUATION: tinfo->get_buffer() "
		              "returned nullptr.");
	}
	...
	// Fill in all the information, and then downgrade as requested by the
	// caller, or raise an error if that's not possible.
	...
	else if ((flags & PyBUF_F_CONTIGUOUS) == PyBUF_F_CONTIGUOUS)
	{
		...
		{
			std::memset(view, 0, sizeof(Py_buffer));
			set_error(PyExc_BufferError, "Fortran-contiguous buffer requested "
			                             "for discontiguous storage");
			return -1;
		}
	}
	...
	// Set these after all checks so they don't leak out into the caller, and
	// can be automatically cleaned up on error.
	view->buf = info->ptr;
