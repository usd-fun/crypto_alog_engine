               ...
               bool need_alias)
{
	...
	// because it does not keep the trampoline Python object alive. For types
	// that don't inherit from enable_shared_from_this it does not matter if
...
void construct(value_and_holder &v_h,
               std::unique_ptr<Alias<Class>, D> &&unq_ptr, bool /*need_alias*/)
{
	auto *ptr = unq_ptr.get();
	no_nullptr(ptr);
...
void setstate(value_and_holder &v_h, std::pair<T, O> &&result, bool need_alias)
{
	...
	// Our tests never run into an unset dict, but being careful here for now
	// (see #5658)
