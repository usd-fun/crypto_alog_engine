...
#error                                                                         \
    "PYBIND11_INTERNALS_VERSION 9 is the minimum for all platforms for pybind11v3."
#endif
...
struct internals
{
	...
	std::unordered_map<PyTypeObject *, std::vector<type_info *>>
	    registered_types_py;
#ifdef Py_GIL_DISABLED
	std::unique_ptr<instance_map_shard[]>
	    instance_shards; // void * -> instance*
	...
	internals()
	{
		...
		{
			pybind11_fail("internals constructor: could not successfully "
			              "initialize the tstate TSS key!");
		}
		...
		{
			pybind11_fail(
			    "internals constructor: could not successfully initialize the "
...
/// We use this counter to figure out if there are or have been multiple
/// subinterpreters active at any point. This must never decrease while any
...
inline std::unique_ptr<InternalsType> *&get_internals_pp()
{
	...
	{
		// Internals is one per interpreter. When multiple interpreters are
		// alive in different threads we have to allow them to have different
		// internals, so we need a thread_local.
		static thread_local std::unique_ptr<InternalsType> *t_internals_pp =
		    nullptr;
		...
		// PyThreadState to check if it is necessary.  The caller will see a
		// null return and do the fetch from the state dict or create a new one
		// (as needed).
...
get_internals_pp_from_capsule_in_state_dict(dict &state_dict,
                                            char const *state_dict_key)
{
	auto internals_obj = reinterpret_steal<object>(
	    dict_getitemstringref(state_dict.ptr(), state_dict_key));
	if (internals_obj)
	{
		void *raw_ptr =
		    PyCapsule_GetPointer(internals_obj.ptr(), /*name=*/nullptr);
		if (!raw_ptr)
		{
			raise_from(PyExc_SystemError,
			           "pybind11::detail::get_internals_pp_from_capsule_in_"
			           "state_dict() FAILED");
			throw error_already_set();
		}
...
PYBIND11_NOINLINE internals &get_internals()
{
	...
	// Cannot use py::gil_scoped_acquire inside get_internals since that calls
	// get_internals.
	gil_scoped_acquire_simple gil;
	error_scope err_scope;

	dict state_dict = get_python_state_dict();
	internals_pp = get_internals_pp_from_capsule_in_state_dict<internals>(
	...
	else
	{
		...
		{
			// This calls get_internals, so cannot be called from within the
			// internals constructor called above because internals_ptr must be
			// set before get_internals is called again
...
{
	// Use the address of this static itself as part of the key, so that the
	// value is uniquely tied to where the module is loaded in memory
...
inline local_internals &get_local_internals()
{
	...
	dict state_dict = get_python_state_dict();
	local_internals_pp =
	    get_internals_pp_from_capsule_in_state_dict<local_internals>(
...
template <typename T> T &get_or_create_shared_data(const std::string &name)
{
	return *detail::with_internals(
	    [&](detail::internals &internals)
	    {
		    auto it = internals.shared_data.find(name);
		    T *ptr =
		        (T *)(it != internals.shared_data.end() ? it->second : nullptr);
		    if (!ptr)
		    {
			    ptr = new T();
