...
inline void global_internals_native_enum_type_map_set_item(
    const std::type_index &enum_type_index, PyObject *py_enum)
{
	with_internals(
	    [&](internals &internals)
                                ...
                                const std::string &append_to_exception_message)
{
	...
	while (std::getline(stream, part, '.'))
	{
		...
		if (!next_scope)
		{
			...
			next_scope = reinterpret_steal<object>(
			    PyImport_ImportModule(next_path.c_str()));
			if (!next_scope)
			{
...
inline void native_enum_data::finalize()
{
	...
	global_internals_native_enum_type_map_set_item(enum_type_index,
	                                               py_enum.release().ptr());
}
