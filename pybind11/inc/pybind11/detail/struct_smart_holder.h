...
inline void builtin_delete_if_destructible(void *)
{
	...
	// raw_ptr;`), but throwing an exception from a destructor will
	// std::terminate the process. Therefore the runtime check for
	// lifetime-management correctness is implemented elsewhere (in
...
struct smart_holder
{
	...
	static void ensure_pointee_is_destructible(const char *context)
	{
		if (!std::is_destructible<T>::value)
		{
			throw std::invalid_argument(
			    std::string("Pointee is not destructible (") + context + ").");
		}
	}
	...
	void ensure_is_populated(const char *context) const
	{
		if (!is_populated)
		{
			throw std::runtime_error(std::string("Unpopulated holder (") +
			                         context + ").");
		}
	}
	void ensure_is_not_disowned(const char *context) const
	{
		if (is_disowned)
		{
			throw std::runtime_error(
			    std::string("Holder was disowned already (") + context + ").");
		}
	}
	...
	void ensure_has_pointee(const char *context) const
	{
		if (!has_pointee())
		{
			throw std::invalid_argument(std::string("Disowned holder (") +
			                            context + ").");
		}
	}
	...
	void reset_vptr_deleter_armed_flag(bool armed_flag) const
	{
		...
		{
			throw std::runtime_error("smart_holder::reset_vptr_deleter_armed_"
			                         "flag() called in an invalid context.");
		}
		vptr_del_ptr->armed_flag = armed_flag;
	}

	// Caller is responsible for precondition: ensure_compatible_rtti_uqp_del<T,
	// D>() must succeed.
	template <typename T, typename D>
	...
	template <typename T> T *as_raw_ptr_unowned() const
	{
	...
	template <typename T>
	static smart_holder
	from_raw_ptr_take_ownership(T *raw_ptr, bool void_cast_raw_ptr = false)
	{
	                                    ...
	                                    void *void_ptr = nullptr)
	{
		...
		hld.vptr_is_using_builtin_delete =
		    is_std_default_delete<T>(*hld.rtti_uqp_del);
		...
		{
			gd = make_guarded_custom_deleter<T, D>(
