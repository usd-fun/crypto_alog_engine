...
// Slightly faster code paths are available when PYBIND11_SUBINTERPRETER_SUPPORT
// is *not* defined, so avoid defining it for implementations that do not
// support subinterpreters. However, defining it unnecessarily is not expected
// to break anything.
...
#define PYBIND11_MODULE(name, variable, ...)                                   \
	...
	static ::pybind11::module_::slots_array PYBIND11_CONCAT(                   \
	    pybind11_module_slots_, name);                                         \
		...
		{                                                                      \
			...
			slots[0] = {Py_mod_exec,                                           \
			            reinterpret_cast<void *>(                              \
			                &PYBIND11_CONCAT(pybind11_exec_, name))};          \
			slots[1] = {0, nullptr};                                           \
			return ::pybind11::module_::initialize_multiphase_module_def(      \
			    PYBIND11_TOSTRING(name), nullptr,                              \
			    &PYBIND11_CONCAT(pybind11_module_def_, name), slots,           \
			    ##__VA_ARGS__);                                                \
		}();                                                                   \
		return result.ptr();                                                   \
	}                                                                          \
...
struct instance
{
	...
	bool has_patients : 1;
	/// If true, this Python object needs to be kept alive for the lifetime of
	/// the C++ value.
	bool is_alias : 1;
...
PYBIND11_RUNTIME_EXCEPTION(cast_error,
                           PyExc_RuntimeError) /// Thrown when pybind11::cast or
                                               /// handle::call fail due to a
                                               /// type casting error
