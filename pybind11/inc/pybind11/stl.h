...
inline bool object_is_convertible_to_std_map(const handle &src, bool convert)
{
	...
	// `__getitem__()` without checking the type of keys. In order to restrict
	// the allowed types closer to actual Mapping-like types, we also check for
	// the `items()` method.
...
public:
	bool load(handle src, bool convert)
	{
		...
		return convert_elements(dict(reinterpret_borrow<iterable>(items)),
		                        convert);
	}
...
template <typename Type, typename Value> struct list_caster
{
	...
	bool load(handle src, bool convert)
	{
		...
		// ensure that the generator is not left in an unpredictable (to the
		// caller) partially-consumed state.
		assert(isinstance<iterable>(src));
		return convert_elements(tuple(reinterpret_borrow<iterable>(src)),
		                        convert);
	}
	...
	static handle cast(T &&src, return_value_policy policy, handle parent)
	{
		...
		for (auto &&value : src)
		{
			auto value_ = reinterpret_steal<object>(value_conv::cast(
			    detail::forward_like<T>(value), policy, parent));
			if (!value_)
			{
				return handle();
			}
...
private:
	...
	bool convert_elements(handle seq, bool convert)
	{
		...
		// Using `resize` to preserve the behavior exactly as it was before PR
		// #5305 For the `resize` to work, `Value` must be default
		// constructible. For `std::valarray`, this is a requirement:
