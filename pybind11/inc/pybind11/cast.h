	...
	bool load(handle src, bool convert)
	{
		...
		if (native_enum)
		{
			...
			value = static_cast<EnumType>(
			    static_cast<Underlying>(underlying_caster));
			return true;
		}
		if (!pybind11_enum_)
		{
...
public:
		...
		else
		{
			...
			{ // signed integer:
				py_value = sizeof(T) <= sizeof(long)
				               ...
				               : (py_type)PYBIND11_LONG_AS_LONGLONG(
				                     src_or_index.ptr());
			}
		}
	...
	static
	    typename std::enable_if<std::is_floating_point<U>::value, handle>::type
	...
	PYBIND11_TYPE_CASTER(T, io_name<std::is_integral<T>::value>(
...
public:
	...
	bool load(handle h, bool)
	{
		...
		const auto &bases =
		    all_type_info((PyTypeObject *)type::handle_of(h).ptr());
		if (bases.size() == 1)
...
protected:
	                        ...
	                        index_sequence<Is...>)
	{
		...
		PYBIND11_WORKAROUND_INCORRECT_GCC_UNUSED_BUT_SET_PARAMETER(policy,
		                                                           parent);
...
protected:
	...
	void load_value(value_and_holder &&v_h)
	{
		...
		throw cast_error(
		    ...
		    "(#define PYBIND11_DETAILED_ERROR_MESSAGES or compile in debug "
		    "mode for "
		    "type information)");
#else
		    "of type '" +
...
public:
	...
	explicit operator std::shared_ptr<type> *()
	{
		...
		{
			pybind11_fail("Passing `std::shared_ptr<T> *` from Python to C++ "
			              "is not supported "
			              "(inherently unsafe).");
		}
	                   ...
	                   return_value_policy policy, handle parent)
	{
		...
		if (st.second->holder_enum_v == detail::holder_enum_t::smart_holder)
		{
			return smart_holder_type_caster_support::
			    smart_holder_from_shared_ptr(src, policy, parent, st);
		}
		return type_caster_base<type>::cast_holder(ptr, &src);
	}

	// This function will succeed even if the `responsible_parent` does not own
	// the wrapped C++ object directly. It is the responsibility of the caller
	// to ensure that the `responsible_parent` has a `keep_alive` relationship
	// with the owner of the wrapped C++ object, or that the wrapped C++ object
	// lives for the duration of the process.
	static std::shared_ptr<type>
	shared_ptr_with_responsible_parent(handle responsible_parent)
	{
		...
		return loader.sh_load_helper.load_as_shared_ptr(loader.value,
		                                                responsible_parent);
	}
...
protected:
	...
	bool try_implicit_casts(handle src, bool convert)
	{
		for (auto &cast : typeinfo->implicit_casts)
		{
			...
			{
				value = cast.second(sub_caster.value);
				if (typeinfo->holder_enum_v ==
				    detail::holder_enum_t::smart_holder)
				{
					sh_load_helper.loaded_v_h =
					    sub_caster.sh_load_helper.loaded_v_h;
				}
				else
				{
...
public:
	                   ...
	                   return_value_policy policy, handle parent)
	{
		...
		{
			return smart_holder_type_caster_support::
			    smart_holder_from_unique_ptr(std::move(src), policy, parent,
			                                 st);
		}
	...
	void load_value(value_and_holder &&v_h)
	{
		...
		pybind11_fail("Passing `std::unique_ptr<T>` from Python to C++ "
		              "requires `py::class_<T, "
	...
	explicit operator const std::unique_ptr<type, deleter> &()
	{
		if (typeinfo->holder_enum_v == detail::holder_enum_t::smart_holder)
		{
			...
			unique_ptr_storage =
			    std::shared_ptr<std::unique_ptr<type, deleter>>(
			        new std::unique_ptr<type, deleter>{
			            sh_load_helper
			                .template load_as_const_unique_ptr<deleter>(
			                    shared_ptr_storage.get())},
			        [](std::unique_ptr<type, deleter> *ptr)
			        {
				        if (!ptr)
				        {
					        pybind11_fail("FATAL: `const std::unique_ptr<T, D> "
					                      "&` was disowned "
...
template <bool Value = false> struct always_construct_holder_value
{
...
{
	static constexpr auto name =
	    const_name("**kwargs: ") + make_caster<T>::name;
};
...
struct return_value_policy_override<
    Return,
    detail::enable_if_t<
        std::is_base_of<type_caster_generic, make_caster<Return>>::value, void>>
{
     ...
     handle parent = handle())
{
	...
	{
		policy =
		    std::is_pointer<no_ref_T>::value ? return_value_policy::reference
...
#else
	operator""_a // clang 17 generates a deprecation warning if there is a
	             // space.
#endif
...
private:
	...
	[[noreturn]] static void multiple_values_error()
	{
		throw type_error("Got multiple values for keyword argument "
		                 "(#define PYBIND11_DETAILED_ERROR_MESSAGES or compile "
