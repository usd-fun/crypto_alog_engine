...
public:
	bool load(handle src, bool convert)
	{
		...
		if (auto cfunc = func.cpp_function())
		{
			...
			{
				function_record *rec =
				    function_record_ptr_from_PyObject(cfunc_self);
				while (rec != nullptr)
				{
					if (rec->is_stateless &&
					                  ...
					                  rec->data[1])))
					{
						struct capture
						{
							...
							{
								return PYBIND11_STD_LAUNDER(
								    reinterpret_cast<capture *>(data));
							}
						};
		...
		value = type_caster_std_function_specializations::func_wrapper<Return,
		                                                               Args...>(
