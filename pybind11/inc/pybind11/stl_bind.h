    ...
    enable_if_t<is_comparable<Vector>::value, Class_> &cl)
{
	...
	cl.def(
	    "count", [](const Vector &v, const T &x)
	    { return std::count(v.begin(), v.end(), x); }, arg("x"),
	...
	cl.def(
	    "__contains__", [](const Vector &v, const T &x)
	    { return std::find(v.begin(), v.end(), x) != v.end(); }, arg("x"),
                ...
                Class_> &cl)
{
	...
	cl.def(
	    "extend", [](Vector &v, const Vector &src)
	    { v.insert(v.end(), src.begin(), src.end()); }, arg("L"),
...
void vector_buffer_impl(Class_ &cl, std::true_type)
{
	...
	cl.def(init(
	    [](const buffer &buf)
	    {
		    ...
		    if (info.ndim != 1 ||
		        info.strides[0] % static_cast<ssize_t>(sizeof(T)))
		    {
			    throw type_error(
                ...
                Class_> &cl)
{
	...
	cl.def("__setitem__",
	       [](Map &m, const KeyType &k, const MappedType &v)
	       {
		       ...
		       if (!r.second)
		       {
			       // value type is not copy assignable so the only way to
			       // insert it is to erase it first...
			       m.erase(r.first);
			       m.emplace(k, v);
		       }
	       });
...
template <typename Map> struct KeysViewImpl : public detail::keys_view
{
	...
	bool contains(const handle &k) override
	{
		try
		{
			return map.find(k.template cast<typename Map::key_type>()) !=
			       map.end();
		}
		catch (const cast_error &)
		{
...
inline str format_message_key_error_key_object(handle py_key)
{
	...
	if (len(message) > 2 * cut_length + 3)
	{
		return str(message[slice(0, cut_length, 1)]) + str("✄✄✄") +
		       str(message[slice(-cut_length,
		                         static_cast<ssize_t>(len(message)), 1)]);
	}
	return message;
}
                                  ...
                                  Args &&...args)
{
	...
	cl.def(
	    ...
	    [](Map &m) {
		    return std::unique_ptr<ValuesView>(
		        new detail::ValuesViewImpl<Map>(m));
	    },
	    keep_alive<0, 1>() /* Essential: keep map alive while view exists */
	);
	...
	cl.def(
	    ...
	    [](Map &m) {
		    return std::unique_ptr<ItemsView>(
		        new detail::ItemsViewImpl<Map>(m));
	    },
	    keep_alive<0, 1>() /* Essential: keep map alive while view exists */
	);
	...
	cl.def("__delitem__",
	       [](Map &m, const KeyType &k)
	       {
		       ...
		       if (it == m.end())
		       {
			       set_error(PyExc_KeyError,
			                 detail::format_message_key_error(k));
			       throw error_already_set();
		       }
		       m.erase(it);
	       });
