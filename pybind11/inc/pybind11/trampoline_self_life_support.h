...
struct trampoline_self_life_support
{
	...
	// (ASAN failures manually verified). The reason is that v_h needs to be
	// kept default-initialized.
	...
	// are).
	trampoline_self_life_support &
	operator=(const trampoline_self_life_support &) = delete;
	trampoline_self_life_support &
	operator=(trampoline_self_life_support &&) = delete;
};
