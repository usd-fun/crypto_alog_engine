...
template <typename Derived> class object_api : public pyobject_tag
{
  ...
  */
	...
	a `handle` or `object` subclass causes a call to ``__setitem__``. \endrst */
	...
	`handle` or `object` subclass causes a call to ``setattr``. \endrst */
	...
	ssize_t ref_count() const
	{
		...
		return static_cast<ssize_t>(
		    static_cast<int>(Py_REFCNT(derived().ptr())));
#else
		return Py_REFCNT(derived().ptr());
#endif
	}

	PYBIND11_DEPRECATED("Call py::type::handle_of(h) or py::type::of(h) "
...
class handle : public detail::object_api<handle>
{
  public:
	...
	/// ``0``.
	template <typename T,
	          detail::enable_if_t<detail::is_pyobj_ptr_or_nullptr_t<T>::value,
	                              int> = 0>
	...
	template <typename T,
	          detail::enable_if_t<
	              detail::all_of<
	                  detail::none_of<std::is_base_of<handle, T>,
	...
	calls this function automatically. Returns a reference to itself. \endrst */
	...
	`cast_error` will be throw upon failure. \endrst */
  ...
  private:
	...
	{
		fprintf(stderr,
		        "%s is being called while the GIL is either not held or "
		        "invalid. Please see "
		        "https://pybind11.readthedocs.io/en/stable/advanced/"
		        "misc.html#common-sources-of-global-interpreter-lock-errors "
		        "for debugging advice.\n"
		        "If you are convinced there is no bug in your code, you can "
		        "#define "
		        "PYBIND11_NO_ASSERT_GIL_HELD_INCREF_DECREF "
		        "to disable this check. In that case you have to ensure this "
		        "#define is consistently "
...
class PYBIND11_EXPORT_EXCEPTION error_already_set : public std::exception
{
  ...
  private:
	...
	///          finalizing.
	static void
	m_fetched_error_deleter(detail::error_fetch_and_normalize *raw_ptr);
};
...
struct list_item
{
	...
	static void set(handle obj, const IdxType &index, handle val)
	{
		...
		if (PyList_SetItem(obj.ptr(), ssize_t_cast(index),
		                   val.inc_ref().ptr()) != 0)
		{
			throw error_already_set();
		}
...
struct tuple_item
{
	...
	static void set(handle obj, const IdxType &index, handle val)
	{
		...
		if (PyTuple_SetItem(obj.ptr(), ssize_t_cast(index),
		                    val.inc_ref().ptr()) != 0)
		{
			throw error_already_set();
		}
...
#else
	operator""_s // clang 17 generates a deprecation warning if there is a
	             // space.
#endif
...
class capsule : public object
{
  public:
	...
	{
		m_ptr = PyCapsule_New(reinterpret_cast<void *>(destructor), nullptr,
		                      [](PyObject *o)
		                      {
  ...
  private:
	...
	{
		m_ptr = PyCapsule_New(const_cast<void *>(value), name,
		                      [](PyObject *o)
		                      {
			                      // guard if destructor called while err
			                      // indicator is set
			                      error_scope error_guard;
			                      auto destructor =
...
template <typename T> class Args : public args
{
	using args::args;
};

template <typename T> class KWArgs : public kwargs
{
	using kwargs::kwargs;
};
  ...
  public:
	...
	/** \rst
	    ...
	    :param readonly: Flag to indicate if the underlying storage may be
	        written to.
	 \endrst */
	static memoryview from_buffer(void *ptr, ssize_t itemsize,
	                              const char *format,
	...
	static memoryview from_buffer(const void *ptr, ssize_t itemsize,
	                              ...
	                              detail::any_container<ssize_t> strides)
	{
		return memoryview::from_buffer(const_cast<void *>(ptr), itemsize,
		                               format, std::move(shape),
