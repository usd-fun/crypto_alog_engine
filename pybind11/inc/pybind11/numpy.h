...
struct npy_api
{
	...
	enum constants
	{
		...
		NPY_UINT64_ = platform_lookup<std::uint64_t, unsigned long,
...
class dtype : public object
{
  public:
	...
	/// Type number of dtype, normalized to match the return value of `num_of`
	/// for equivalent types. This function can be used to write switch
	/// statements that correctly handle equivalent types with different type
	/// numbers.
	...
	dtype strip_padding(ssize_t itemsize)
	{
		...
		for (auto field : field_dict.attr("items")())
		{
			...
			field_descriptors.emplace_back(
			    std::move(name), format.strip_padding(format.itemsize()),
			    std::move(offset));
		}
...
template <typename T>
struct npy_format_descriptor_name<T,
                          ...
                          bool (*direct_converter)(PyObject *, void *&))
{
	...
	numpy_internals.registered_dtypes[tindex] = {dtype_ptr,
	                                             std::move(format_str)};
	with_internals(
	    [tindex, &direct_converter](internals &internals)
...
#define PYBIND11_NUMPY_DTYPE_EX(Type, ...)                                     \
	::pybind11::detail::npy_format_descriptor<Type>::register_dtype(           \
	    ::std::vector<::pybind11::detail::field_descriptor>{                   \
	        PYBIND11_MAP2_LIST(PYBIND11_FIELD_DESCRIPTOR_EX, Type,             \
	                           __VA_ARGS__)})
...
// NVCC for some reason breaks if NVectorized is private
  ...
  private:
	...
	// is manually inlined.
	using arg_call_types =
	    std::tuple<typename vectorize_arg<Args>::call_type...>;
	...
	void apply_broadcast(std::array<buffer_info, NVectorized> &buffers,
	                     std::array<void *, N> &params, Return *out,
	                     size_t size, const std::vector<ssize_t> &output_shape,
	                     index_sequence<Index...>, index_sequence<VIndex...>,
	                     index_sequence<BIndex...>)
	{
